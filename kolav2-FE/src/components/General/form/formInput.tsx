...
const FormInput = ({
  ...
  maxLength,
  defaultValue,
  setDefaultOnMount = false,
  ...rest
}: iInputField) => {
  const isLeftIcon = iconPosition === "left";

  useEffect(() => {
    if (setDefaultOnMount && defaultValue && !value && onChange && name) {
      const valueToSet =
  ...
  }, [setDefaultOnMount, defaultValue, value, onChange, name, type]);
    ...
    <div>
      ...
      <div className="relative flex-1">
        ...
        <input
          ...
          readOnly={readOnly}
          maxLength={maxLength}
          className={`${className} block px-${paddingX ? paddingX : "4"} py-${
            paddingY ? paddingY : "2"
          }
