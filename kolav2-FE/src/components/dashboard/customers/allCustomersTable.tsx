...
import { displayValue, formatPhoneNumber } from "@/utils/helpers";
...
const AllCustomersTable = ({
  ...
  const columns = [
    ...
    {
      header: "Customer ID",
      accessorKey: "customer_custom_id",
    },
    {
        ...
        return (
          <span className="text-gray_5">
            {row.original?.customer_business_type
              ? Array.isArray(row.original.customer_business_type)
                ? row.original.customer_business_type.join(", ")
                : row.original.customer_business_type
              : "N/A"}
          </span>
        );
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        const phoneNumber = row.original?.customer_phone;
        const formattedPhone = phoneNumber
          ? formatPhoneNumber(phoneNumber)
          : "N/A";

        return (
          <Typography className="text-gray_5">
