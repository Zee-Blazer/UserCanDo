...
interface OutForDeliveryOrdersUpdatedProps {
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

const OutForDeliveryOrdersUpdated = ({ onRefresh, isRefreshing }: OutForDeliveryOrdersUpdatedProps) => {

  const router = useRouter();
  const dispatch = useDispatch();
  const { outForDeliveryOrders } = useDashboardSelector();
  const { loadOrdersByStatus, isOrdersLoading, refreshAllOrdersData } = useDash();

  // Load out for delivery orders on mount
  useEffect(() => {
  ...
  const orders = outForDeliveryOrders && outForDeliveryOrders.length > 0 ? outForDeliveryOrders : [
    {
      id: 1,
...
export default OutForDeliveryOrdersUpdated;
