...
interface DeliveryOrdersUpdatedProps {
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

const DeliveryOrdersUpdated = ({ onRefresh, isRefreshing }: DeliveryOrdersUpdatedProps) => {

  const router = useRouter();
  const dispatch = useDispatch();
  const { deliveredOrders } = useDashboardSelector();
  const { loadOrdersByStatus, isOrdersLoading, refreshAllOrdersData } = useDash();

  // Load delivered orders on mount
  useEffect(() => {
  ...
  const orders = deliveredOrders && deliveredOrders.length > 0 ? deliveredOrders : [
    {
      id: 1,
...
export default DeliveryOrdersUpdated;
