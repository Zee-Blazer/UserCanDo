...
interface ApprovedOrdersUpdatedProps {
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

const ApprovedOrdersUpdated = ({ onRefresh, isRefreshing }: ApprovedOrdersUpdatedProps) => {

  const router = useRouter();
  const dispatch = useDispatch();
  const { approvedOrders } = useDashboardSelector();
  ...
  const orders = approvedOrders && approvedOrders.length > 0 ? approvedOrders : [
    {
      id: 1,
...
export default ApprovedOrdersUpdated;
