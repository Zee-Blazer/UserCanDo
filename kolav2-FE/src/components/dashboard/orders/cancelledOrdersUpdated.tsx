...
interface CancelledOrdersUpdatedProps {
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

const CancelledOrdersUpdated = ({ onRefresh, isRefreshing }: CancelledOrdersUpdatedProps) => {

  const router = useRouter();
  const dispatch = useDispatch();
  const { cancelledOrders } = useDashboardSelector();
  const { loadOrdersByStatus, isOrdersLoading, refreshAllOrdersData } = useDash();

  // Load cancelled orders on mount
  useEffect(() => {
  ...
  const orders = cancelledOrders && cancelledOrders.length > 0 ? cancelledOrders : [
    {
      id: 1,
    ...
    },
    {
        ...
        sales_agent_name: 'Kofi Mensah',
        total_sale_amount: 1200.5,
        sale_type: 'Cash Sale',      status: 'Cancelled',
    },
  ];
...
export default CancelledOrdersUpdated;
