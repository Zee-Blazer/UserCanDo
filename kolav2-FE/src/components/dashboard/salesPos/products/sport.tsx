...
const Sport = ({ addToCart }: any) => {
  const { loadProductsByCategory } = useDash();
  const { sportsFitnessProducts } = useDashboardSelector();
  ...
  if (
    !Array.isArray(sportsFitnessProducts) ||
    sportsFitnessProducts.length === 0
  ) {
  ...
  return (
    <main>
      <div className="grid gap-4">
        {sportsFitnessProducts?.map((product, index) => {
          return (
            <div
              key={index}
